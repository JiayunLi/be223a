...
public class OWLIMOntology 
  ...
  public void load() throws ResourceInstantiationException {
    try {
      ...
      OntologyFormat ontologyFormat = OntologyFormat.RDFXML;
      if(rdfXmlURL != null && rdfXmlURL.toString().trim().length() > 0) {
        ...
        ontologyFormat = OntologyFormat.NTRIPLES;
      }
      else if(n3URL != null && n3URL.toString().trim().length() > 0) {
        ontologyURL = n3URL;
        ontologyFormat = OntologyFormat.N3;
      }
      else if(turtleURL != null && turtleURL.toString().trim().length() > 0) {
        ontologyURL = turtleURL;
        ontologyFormat = OntologyFormat.TURTLE;
      }
      else {
        ontologyURL = null;
        ontologyFormat = OntologyFormat.RDFXML;
      }
